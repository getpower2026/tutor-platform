...
import { Star, Clock, GraduationCap, BookOpen, Calendar, MessageSquare, Phone } from "lucide-react";
...
export default function TeacherDetailPage() {
  ...
  if (!teacher || teacher.message) return (
    <div className="min-h-screen"><Navbar /><div className="text-center py-20 text-gray-400">找不到老師</div></div>
  );
  ...
  return (
    <div className="min-h-screen">
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 text-amber-500 mb-3">
                ...
                {teacher.reviewCount > 0 && <span className="text-gray-400">({teacher.reviewCount} 則評價)</span>}
              </div>
            ...
            <div className="text-right">
              ...
              <button onClick={() => session ? setShowBooking(true) : router.push("/login")} className="btn-primary px-8 py-3 text-base">
                <Calendar className="w-4 h-4 mr-2" />立即預約
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="card p-6">
              <h2 className="font-bold text-lg mb-4 flex items-center gap-2">
                <MessageSquare className="w-5 h-5 text-primary-500" />學生評價
                {reviews.length > 0 && <span className="text-sm font-normal text-gray-400">（{reviews.length} 則）</span>}
              </h2>
              {reviews.length === 0 ? (
                  ...
                  {reviews.map((review) => (
                    <div key={review.id} className="border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          <div className="font-medium text-sm">{review.reviewer.name[0]}{"*".repeat(Math.max(0, review.reviewer.name.length - 1))}</div>
                      ...
                      {review.comment && <p className="text-gray-600 text-sm leading-relaxed pl-11">{review.comment}</p>}
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="card p-6 h-fit">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="border-t pt-3 text-xs text-gray-400">實際收費由老師與家長自行討論，本平台完全免費。</div>
            </div>
            <button onClick={() => session ? setShowBooking(true) : router.push("/login")} className="btn-primary w-full mt-4">
              選擇時段
            </button>
          </div>
        </div>
      ...
      {showBooking && <BookingModal teacher={teacher} onClose={() => setShowBooking(false)} />}
    </div>
  );
}
