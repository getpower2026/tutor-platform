...
export default function RoomPage() {
  ...
  const excalidrawUrl = getExcalidrawUrl(id);
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#111", overflow: "hidden" }}>
      ...
      <div style={{ background: "#1f2937", padding: "6px 12px", display: "flex", alignItems: "center", gap: "8px", flexShrink: 0 }}>
        ...
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: "4px", padding: "5px 10px", background: tab === "video" ? "#4f46e5" : "#374151", color: "white", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "13px" }}
        >
          <Video style={{ width: 14, height: 14 }} /> 視訊
        </button>
        <button
          onClick={() => setTab("whiteboard")}
          style={{ display: "flex", alignItems: "center", gap: "4px", padding: "5px 10px", background: tab === "whiteboard" ? "#7c3aed" : "#374151", color: "white", border: "none", borderRadius: "6px", cursor: "pointer", fontSize: "13px" }}
        >
      ...
      {/* 內容區：兩個 iframe 都常駐，用 display 切換 */}
            ...
            {isTeacher ? (
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  ...
                  <button onClick={() => router.push("/dashboard")} style={{ padding: "11px", background: "#f3f4f6", color: "#374151", fontSize: "13px", border: "none", borderRadius: "12px", cursor: "pointer" }}>
                    直接離開（不標記完成）
                  </button>
                  <button onClick={() => setLeaveModal(false)} style={{ padding: "8px", background: "transparent", color: "#9ca3af", fontSize: "12px", border: "none", cursor: "pointer" }}>
                    取消，繼續上課
                  </button>
                </div>
              </>
              ...
              <>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  <button onClick={() => router.push("/dashboard")} style={{ padding: "13px", background: "#ef4444", color: "white", fontWeight: "bold", fontSize: "15px", border: "none", borderRadius: "12px", cursor: "pointer" }}>
                    離開教室
                  </button>
                  <button onClick={() => setLeaveModal(false)} style={{ padding: "8px", background: "transparent", color: "#9ca3af", fontSize: "12px", border: "none", cursor: "pointer" }}>
                    取消，繼續上課
